...
function splitBalancedByTeam(pairs) {
  ...
  for (const [, teamPairs] of teamMap.entries()) {
    const shuffled = shuffle(teamPairs);
    shuffled.forEach((pair, idx) => {
      groups[idx % 2].push(pair);
    });
  }
...
function ensureKnockout(tournament) {
  if (!allGroupMatchesFinished(tournament)) {
  ...
  return {
    ...
    tournament: {
      ...
      groups: tournament.groups.map((group) => ({
        ...
        matches: group.matches.map((round) => round.map((m) => ({ ...m })))
      })),
