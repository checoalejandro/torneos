...
function splitBalancedByTeam(pairs, numGroups = 2) {
  const groups = Array.from({ length: numGroups }, () => []);
  const confirmedPairs = pairs.filter(p => p.confirmed);
  const teams = [...new Set(confirmedPairs.map(p => p.team))];
  ...
  // Distribuimos alternando entre los grupos disponibles
  let gIdx = 0;
  for (const team of teams) {
    const list = teamMap.get(team);
    while (list.length > 0) {
      const pair = list.shift();
      groups[gIdx % numGroups].push(pair);
      gIdx += 1;
    }
  }
...
function buildTournament(pairs, oneGroup = false) {
  const numGroups = oneGroup ? 1 : 2;
  const groupsRaw = splitBalancedByTeam(pairs, numGroups);
  const groups = groupsRaw.map((groupPairs, idx) => ({
    id: idx + 1,
    name: oneGroup ? 'Grupo Único' : `Grupo ${idx + 1}`,
    court: oneGroup ? null : idx + 1,
    pairs: groupPairs,
...
function computeAdvancers(tournament) {
  ...
  if (tournament.oneGroup) {
    const s = ranked[0].standings;
    return [{
      groupId: 1,
      first: s[0] || null,
      second: s[1] || null,
...
function ensureKnockout(tournament) {
  ...
  if (shouldGenSemis) {
    if (tournament.oneGroup) {
      const g = advancers[0];
      if (g?.first && g?.second && g?.third && g?.fourth) {
...
function publicState(state) {
  ...
  if (!tournament) {
    return {
      ...
      tournament: null,
      oneGroup: Boolean(state.oneGroup),
      groupStandings: [],
      liveMatches: [],
      semis: [],
      final: null,
  ...
  return {
    ...
    oneGroup: Boolean(tournament.oneGroup || state.oneGroup),
  };
}
